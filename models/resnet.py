...
class fi_Bottleneck(nn.Module):
    ...
    def __init__(self, ber, bit, inplanes, planes, stride=1, downsample=None):
        super(fi_Bottleneck, self).__init__()
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = conv2d_fi(planes, planes, 3, ber, bit, stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(planes * 4)
        self.relu = nn.ReLU()               
...
class fi_win_Bottleneck(nn.Module):
    ...
    def __init__(self, ber, bit, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = winconv2d_fi(planes, planes, 3, tiles, ber, bit, stride, padding=1,bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(planes * 4)
        self.relu = nn.ReLU()     
...
class fi_fft_Bottleneck(nn.Module):
    ...
    def __init__(self, ber, bit, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = FFTConv2d_fi(planes, planes, 3, ber, bit, stride, padding=1, bias=False)        
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(planes * 4)
        self.relu = nn.ReLU()
...
class ResNet(nn.Module):
    ...
    def __init__(self, ber, bit, block_1, block_2, layers, num_classes=1000):
        ...
        self.layer1 = self._make_layer(block_1, block_2, 64, layers[0], stride=1)
    ...
    def _make_layer(self, block_1, block_2, planes, blocks, stride):
        downsample = None
        ...
        for i in range(1, blocks):
            layers.append(block_1(self.ber, self.bit, self.inplanes, planes, 1))
...
def fi_resnet50(ber, bit, pretrained=False, **kwargs):
    ...
    model = ResNet(ber, bit, fi_Bottleneck, Bottleneck, [3, 4, 6, 3], **kwargs)
...
def fi_fft_resnet50(ber, bit, pretrained=False, **kwargs):

    model = ResNet(ber, bit, fi_fft_Bottleneck, Bottleneck, [3, 4, 6, 3], **kwargs)
